...
describe('#' + namespace, () => {
    ...
    it('Alice can submit a reply for a message', async () => {
        ...
        // Submit the reply.
        const myReply = factory.newTransaction(namespace, 'sendPublicReply');
        myReply.parentMessage = factory.newRelationship(namespace,'Message','1');
        ...
        // Get the asset.
        const assetRegistry = await businessNetworkConnection.getAssetRegistry(namespace + '.' + 'Reply');
        const asset1 = await assetRegistry.get('51');
    ...
    it('Alice can update the subject of her message', async () => {
        ...
        myTrans.oldMessage = factory.newRelationship(namespace,'Message','1');
    ...
    it('Alice can not update the subject of Bob\'s message', async () => {
        ...
        // Submit the reply.
        const myTrans = factory.newTransaction(namespace, 'updateSubject');
        myTrans.oldMessage = factory.newRelationship(namespace,'Message','2');
        myTrans.newSubject = 'Hello Everybody!';
    ...
    it('Alice can update the value of her message', async () => {
        // Use the identity for Alice.
        await useIdentity(aliceCardName);

        // Submit the reply.
        const myTrans = factory.newTransaction(namespace, 'updateValue');
        myTrans.oldMessage = factory.newRelationship(namespace,'Message','1');
        myTrans.newValue = 'This is not a test';
        await businessNetworkConnection.submitTransaction(myTrans);
        ...
        // Validate the asset.
        asset1.creator.getFullyQualifiedIdentifier().should.equal(participantNS + '#alice@email.com');
        asset1.value.should.equal('This is not a test');
        ...
        event.timestamp.should.be.an.instanceOf(Date);
        event.oldMessage.should.equal(myTrans.oldMessage.getFullyQualifiedIdentifier());
        event.oldValue.should.equal('This is a test');
        event.newValue.should.equal('This is not a test');
    });
    ...
    it('Bob can update the subject of his message', async () => {
        ...
        event.timestamp.should.be.an.instanceOf(Date);
        event.oldMessage.should.equal(myTrans.oldMessage.getFullyQualifiedIdentifier());
        event.oldValue.should.equal('Another test message from bob');
        event.newValue.should.equal('This is not a test');
    });

    it('Bob can not update the value of Alice\'s message', async () => {
        // Use the identity for Bob.
        await useIdentity(bobCardName);
